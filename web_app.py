...
def checkout():
    if request.method == 'POST':
        ...
        if not customer_id or not equipment_id:
            flash('Please select both customer and equipment.')
    ...
    if customer_search:
        ...
        cursor.execute("SELECT id, name FROM customers ORDER BY name LIMIT 100")
    customers_list = cursor.fetchall()
...
def checked_out():
    ...
    checked_out_list = cursor.fetchall()
    conn.close()
    return render_template('checked_out.html', checked_out=checked_out_list, search=search)
